...
- Keeping the two inputs we can use a barplot.

--

- **Note:** 

- We are using functions from another package to help with the data (`dplyr`)

- With `plotly` we need to count the data before plotting, not the same in all packages.


--

- **Exercise**: What would happen if you remove `color = ~cut`? Find out. Can you change the default single colour? *Hint: `color = I("choice")`*. 
...
- **EXercise:** Change things so that the code checks if `input$plot_type == "Scatterplot"` it draws a scatterplot `else` it draws a barplot.
...
- We can also display different plots in different tabs. 

- Need two plotting functions and two tabs
...
- **Exercise:** There is some redundancy in this app, identify the repeated lines. We will fix this in a minute.

---
# App 09 Data Processing

- Rather than repeating same processing, can process once & plot twice.

- We wrap the processing in `reactive({})` and then call the reactive function when needed.

---
# App 10 Reading in Data

- Last thing we're going to look at today is doing this with your own data.
